== Rails Assignment

==== Scenario

Your team is working with a golf club management company. As part of an enterprise-wide update the company would like a Rails app for booking tee times at their various clubs. Your job is to prototype the application over several iterations. Acceptance criteria will be provided for each iteration.

==== Your Deliverable

This is not a design assignment. While the app should be usable, it needn't look pretty. If you wish to use something like Twitter Boostrap that's perfectly acceptable.

This exercise is not designed to take a long time. How much time you spend is totally up to you, but do not feel obligated to spend more than a few hours. With that said, your code should be of good quality: something that could easily be used as a base for a more complex production application. We are looking for good code design and therefore the use of design patterns is encouraged. We also encourage unit tests.

Please submit the project as a git repo.

=== Iteration 1

Build a prototype Rails app which allows a user to book a tee-time at a single golf club.

Acceptance criteria:
* Tee times are separated by 20 minutes.
* Only one user may book a given twenty-minute slot.
* The golf club is open for bookings from 9am to 5pm every day.
* Users should have some mechanism to see if a time is already booked.
* Users should be able to cancel a booking.

=== Iteration 2

The prototype has been approved by the management company. Please extend the application to support multiple golf clubs.

Acceptance criteria:
* The application should support multiple golf clubs. A user can make a booking at any club.

=== Iteration 3

Management would like to add some constraints to the booking process.

Acceptance criteria:

* A given user should only be able to have a maximum of 2 concurrent bookings.
* Bookings can't be canceled when they start less than an hour from the current time.



= Requirements

- Pow
- Postgres
- Ruby

== Install

    git clone git@github.com:aaronprice/the_greens.git
    cd the_greens
    # Create your config/database.yml
    gem install bundler
    bundle
    rake db:migrate
    # Symlink the app to Pow
    cd ~/.pow
    ln -s /path/to/the_greens thegreens
    # Go to your browser and navigate to http://www.thegreens.dev/

As of Iteration 2, you'll need
    rake db:seed

I've tagged the iterations so you can check them out separately.


== Notes

I've taken heart to the phrase "This is not a design assignment". The UI is terrible, but it is functional.

I chosen deliberatly not to have passwords for users. Simply because when you're testing on the front end, it's much easier without the hassle of a password. In a production environment, this would not be the case.

I deliberately used validation in the form for the reservation because business logic and data integrity logic are different.

I'll be happy to answer any other question you might have.

Also, I'd love your feedback on the assignment in general. What you liked, didn't like. Anything, really.
